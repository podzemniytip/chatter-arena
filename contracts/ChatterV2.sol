...
contract ChatterV2 {
    ...
    uint256 public comDuration;
    uint256 public revDuration;
    ...
    constructor(uint256 _entryFee, uint256 _reg, uint256 _com, uint256 _rev) {
        owner = msg.sender;
        regDuration = _reg;
        comDuration = _com;
        revDuration = _rev;
        _startSeason(_entryFee);
    }

    function _startSeason(uint256 _fee) internal {
        seasonId++;
        Season storage s = seasons[seasonId];
        s.phase = Phase.Registration;
        s.entryFee = _fee;
        s.regEnd = block.timestamp + regDuration;
        s.comEnd = block.timestamp + regDuration + comDuration;
        s.revEnd = block.timestamp + regDuration + comDuration + revDuration;
        emit NewSeason(seasonId, _fee, s.regEnd, s.comEnd, s.revEnd);
    }

    function setDurations(uint256 _reg, uint256 _com, uint256 _rev) external {
